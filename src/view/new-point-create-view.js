...
function createEventTypeItemTemplate(elem) {
  const { name } = elem;
  return (`
...
const typeItemTemplate = createEventTypeItemTemplate();//пока что без аргументов

function createSectionOffersTemplate(element) {
  const { name, title, price} = element;
  return (`
...
const sectionOffersTemplate = createSectionOffersTemplate();//пока что без аргументов
